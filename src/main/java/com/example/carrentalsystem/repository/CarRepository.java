...
public interface CarRepository extends JpaRepository<Car, Long> {
    ...
    @Query("SELECT c FROM Car c WHERE c.id IN (SELECT i.id FROM Car i WHERE i.id NOT IN (SELECT r.car.id FROM Reservation r WHERE r.hireDate <= :to AND r.returnDate >= :from AND r.car.id = i.id)) AND c.brand IN :brands AND c.carType In :carTypes")
    public List<Car> findCarBetweenDatesAndBrandAndCarType(LocalDate from, LocalDate to, @Param("brands") List<Brand> brands, @Param("carTypes") List<CarType> carTypes);

    @Query("SELECT c FROM Car c WHERE c.id IN (SELECT i.id FROM Car i WHERE i.id NOT IN " +
            "(SELECT r.car.id FROM Reservation r WHERE r.hireDate <= :to AND r.returnDate >= :from AND r.car.id = i.id)) " +
            "AND c.brand IN :brands AND c.carType In :carTypes")
    public Page<Car> findCarBetweenDatesAndBrandAndCarType(LocalDate from, LocalDate to, @Param("brands") List<Brand> brands, @Param("carTypes") List<CarType> carTypes, Pageable pageable);
